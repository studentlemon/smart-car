...
void DMA_Source2BUFF_Init(DMA_CHn CHn, void *SADDR, void *DADDR, DMA_sources DMSour, DMA_BYTEn byten,    //
                          uint32_t count,uint8_t SOFF,uint8_t DOFF, DMA_PORTx2BUFF_cfg cfg)
{
   ...
   // 配置 DMA 传输结束后的操作 
    ...
    {
      DMA_SLAST(CHn)      =   (uint32_t)( (cfg & 0x20) == 0 ? (-(count))  : 0 ); //调整目的地址的附加值,主循环结束后恢复目的地址或者保持地址
    }
    if(DOFF == 0)
    {
      DMA_DLAST_SGA(CHn)  = 0 ;
...
//    DMA_SLAST(CHn)      = (uint32_t)SADDR;
//    DMA_DLAST_SGA(CHn)  = (uint32_t)DADDR;
    DMA_CSR(CHn)        =   (0
...
*  备    注：
*************************************************************************/
void DMA_count_Init(DMA_CHn CHn, PTxn ptxn, uint32_t count, DMA_Count_cfg cfg)
{
