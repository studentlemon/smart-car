...
void Key_Init(void)
{   
  gpio_Interrupt_init(PTD7,GPI_DOWN,  RING) ;
  gpio_Interrupt_init(PTD8, GPI_DOWN,  RING) ;
  gpio_Interrupt_init(PTD9, GPI_DOWN,  RING) ;
  gpio_Interrupt_init(PTD10, GPI_DOWN, RING) ;
  gpio_Interrupt_init(PTD11, GPI_DOWN, RING) ;      
}
/*********************************************************** 
函数名称：Key_print 
函数功能：显示角度和加速度初始化
入口参数：AngleCharacter :角度显示字符
          GryoCharacter :加速度显示字符
出口参数：无 
备 注： 
***********************************************************/
...
{   
  OLED_Fill(0x00);//黑屏 
  Dly_ms(1500); 
  OLED_Print_Str(5,0,"BUAA");  //汉字字符串显示
