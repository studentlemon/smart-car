...
//#define OLED_SCL  PTE5_OUT
//#define OLED_SDA  PTE2_OUT
//#define OLED_RST  PTE3_OUT
//#define OLED_DC   PTE6_OUT
//#define OLED_CS   PTE4_OUT
...
void OLED_Init(void)
{
  ...
  OLED_Set_Pos(0,0);
   ...
   OLED_Print_Str(5,2,"BUAA");  //汉字字符串显示
   OLED_Print_Str(5,4,"BUAA");  //汉字字符串显示
   OLED_Print_Str(5,6,"BUAA");  //汉字字符串显示
...
void OledPrint(uint16_t ADdataCount,uint16_t LineCount,uint16_t PredatorFAV, uint8 *dataAD )
{
  ...
  if(temp1){
    for(temp3 = 0 ; temp3 < 16;temp3++){
      temp1 = temp3*PCount*ADdataCount + temp0;
      for(temp4= temp0 ;temp4 < (temp0+128) ;temp4 ++){
        poled[temp5++] = dataAD[temp1 ++];
      }
    }
  }
  else{
    ...
    temp1 = 0 ;
    for(; temp3 < (16) ;temp3 ++){   
      temp5 = temp1*PCount*ADdataCount + temp0;
      temp1++ ;
      for(temp4= 0 ;temp4 < (128) ;temp4 ++){
        poled[OLECount] = dataAD[temp5++] ;
        OLECount ++ ;
      }}
  }
 ...
 //   temp0 = plineCount*128 + 1;
    PCount = 0;
    OLED_Set_Pos(PCount,plineCount);
    temp0 += 128 ;
    temp3 = 128+temp0;
